...
- `asymTorusIso_interacting_second_moment_density_transfer` — interacting 2nd moment bounded
  by the free Gaussian 2nd moment (Cauchy–Schwarz density transfer + Gaussian 4th moment).
- `asymTorusIso_interacting_limit_exists` — Prokhorov UV limit at fixed `(Lt, Ls)`.
- `asymTorusIso_limit_satisfies_OS2` / `asymTorusIso_limit_satisfies_OS` — OS2 and OS0–OS2 of the limit.
- `asymTorusIso_measureHasGreenMomentBound` — Green-controlled exponential moment from a Nelson cutoff.

Sampling bridges live in `AsymSamplingBridge.lean`. Cutoff-family OS assembly, the
volume-uniform axiom, and `cylinderIso_OS_of_RP_OS2` live in `AsymCutoffFamilyOS.lean`.
...
theorem asymTorusIso_limit_satisfies_OS
  ...
  obtain ⟨htrans, hrefl⟩ :=
    asymTorusIso_limit_satisfies_OS2 Lt Ls P mass hmass Nt Ns a hNt hNs ha
      hvolt hvols ha0 μ φ hφ hconv
  exact
    { os0 := asymTorusOS0_of_greenMomentBound Lt Ls mass hmass K C μ hgreen
      ...
      os2_translation := htrans
      os2_timeReflection := hrefl }
